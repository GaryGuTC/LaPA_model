...
def init_weights_adapter(module):
    if isinstance(module, (nn.Linear, nn.Embedding)):
...
def frozen_weights(module):
    for param in module.parameters():
        param.requires_grad = False

def frozen_weights_cs_attn(module):
    for each in list(module):
        for param in each.parameters():
            param.requires_grad = False
        for param in each.crossattention.output.adapter.parameters(): # not freeze adapter
